...
# Modifications: Integrated teacher-student loss setup, added support for SMPL ground truth in prep_gt,
#                added function for counting model parameters 
...
#from datasets.threedpw import THREEDPW, collate_fn
from datasets.agora import AGORA, collate_fn
...
class Trainer(object):
    def __init__(self, model, loss, optimizer, scheduler, device, args, teacher_model=None, best_val=1e5):
        self.model = model
        self.loss = loss
        self.device = device
        self.args = args
        ...
        self.smplx_neutral_11 = smplx.create(SMPLX_DIR, 'smplx', gender='neutral', use_pca=False, flat_hand_mean=False, num_betas=11).to(self.device)
    ...
    def prepare_gt(self, y, patch_size):
        target = {}
        ...
        n_patch = args.img_size // patch_size
        ...
        if has_smplx_params:
            ...
            target['rotvec'] = torch.cat([y['smpl_root_pose'],
                                        y['smpl_body_pose']],2)[idx_h[0],idx_h[1]]
            target['rotmat'] = roma.rotvec_to_rotmat(target['rotvec'])
            target['shape'] = y['smpl_shape'][idx_h[0],idx_h[1]]
    ...
    def train_n_iters(self, data):
        ...
        for i, (x,y) in enumerate(tqdm(data)):
            ...
            gt = self.prepare_gt(y=y, patch_size=self.model.patch_size)
            if self.teacher_model is not None:
                gt_teacher = self.prepare_gt(y=y, patch_size=self.teacher_model.patch_size)
            ...
            with torch.cuda.amp.autocast(enabled=bool(args.amp)):
                ...
                if self.teacher_model is not None and self.use_distill:
                    with torch.no_grad():
                        teacher_pred = self.teacher_model(x, is_training=True, 
                                      idx=gt_teacher['idx'],
                                      K=gt_teacher['K'], 
                                      )
                    
                    # Compute the divergence between the two predictions
                    divergence, div_dict = self.loss(pred, teacher_pred, epoch=self.current_epoch, img_size=self.args.img_size, distill=True)
                    dict_loss['divergence'] = divergence

                    # Add the divergence to the loss
                    alpha = 0.3
                    loss = (1 - alpha) * loss + alpha * divergence
                else:
                    dict_loss['divergence'] = torch.tensor(0.0).to(self.device)
                ...
                if i % self.args.log_freq == 0:
                    ...
                    #print(f"EPOCH={self.current_epoch:03d} - i={i:05d}/{len(data):05d} - workload/ratio_data={meters['workload/ratio_data'].avg:.2f} - loss={meters['loss/total'].avg:.2f} - bce={meters['loss/bce'].avg:.2f} - v3d={meters['loss/v3d'].avg:.2f} - transl={meters['loss/transl'].avg:.2f} - divergence - {meters['loss/divergence'].avg:.2f}")
    ...
    def evaluate(self, data):
        ...
        for i, (x,y) in enumerate(tqdm(data)):
            ...
            if len(bestMatch) > 0:
                for (pid, gid) in bestMatch:                    
                    ...
                    if data.dataset.name == '3dpw':
                        if 'J_regressor_h36m' not in self.__dict__:
            ...
            # visu
            if (self.args.visu_interval and self.args.visu_to_save > 0 and i < self.args.visu_to_save) or (self.args.visu_interval and i % self.args.visu_interval == 0):
                # image
...
def main(args):
    ...
    model = TinyViTModel(pretrained_backbone=1, **vars(args))
    ...
    '''teacher_model = Model(pretrained_backbone=1, **vars(args))
    teacher_model = teacher_model.to(device)

    if args.pretrained_teacher is not None and os.path.isfile(args.pretrained_teacher):
        print(f"Loading teacher weights from {args.pretrained_teacher}")
        ckpt = torch.load(args.pretrained_teacher, weights_only=False)
        log = teacher_model.load_state_dict(ckpt['model_state_dict'], strict=False)
        print(f"{log}")'''
    ...
    trainer = Trainer(model=model, loss=loss, optimizer=optimizer, teacher_model=None, device=device, args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_data', type=str, default='AGORA')
    ...
    parser.add_argument('--val_data', type=str, nargs='+', default=['AGORA'])
    parser.add_argument('--val_split', type=str, nargs='+', default=['test'])
    parser.add_argument('--val_n', type=int, nargs='+', default=[-1])
    parser.add_argument('--val_subsample', type=int, nargs='+', default=[1])
    parser.add_argument('--save_dir', type=str, default='logs/agora')
    parser.add_argument('--name', type=str, default='test_tinyvit_5m_test_set')
    parser.add_argument('--pretrained', type=str, default='logs/agora/train_finetune_tinyvit_5m_1e-5_threedpw_bedlam/checkpoints/00014.pt')
    ...
    parser.add_argument("--learning_rate", "-lr", type=float, default=1e-5, help='learning rate (absolute lr)')
    parser.add_argument('--num_workers', type=int, default=7) 
    parser.add_argument('--eval_only', type=int, default=1, choices=[0,1])
    ...
    parser.add_argument('--visu_interval', type=int, default=1000)
