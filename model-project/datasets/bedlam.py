...
class BEDLAM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        annot['persons'] = [humans[h_idx] for h_idx in sorted_indices]
...
def collate_fn(x, *args, **kwargs):
    ...
    y['n_humans'] = torch.from_numpy(np.stack([len(x[i][1]['persons']) for i in range(bs)])).float() # [bs]
    max_persons = int(max(y['n_humans']))

    # Validity index
    l_valid_humans = []
    for i in range(bs):
        n_humans = len(x[i][1]['persons'])
    ...
    key2shape = {}
    for i in range(bs):
        for h in range(len(x[i][1]['persons'])):
            keys = list(x[i][1]['persons'][h].keys())
            for k in keys:
                if isinstance(x[i][1]['persons'][h][k], np.ndarray):
                    all_keys.append(k)
                    key2shape[k] = x[i][1]['persons'][h][k].shape
                else:
                    pass
...
if __name__ == "__main__":
    dataloader(split='training', batch_size=4, num_workers=8, shuffle=1,
               extension='png', img_size=512, n=-1, res=None, n_iter=1000)