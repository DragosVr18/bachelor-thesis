...
# Modifications: Added support for SMPL ground truth data (for example, from 3DPW dataset)
...
from utils import SMPLX2SMPL_REGRESSOR
import pickle
...
class Loss(torch.nn.Module):
    ...
    def forward(self, y_hat, y, epoch=None, img_size=None, distill=False):
        
        # Detection
        y_scores = y['scores']
        y_hat_scores = y_hat['scores']
        if not distill:
          y_scores = (y_scores >= 1).to(int).unsqueeze(-1)
        elif distill and y_scores.shape != y_hat_scores.shape:
          y_scores_perm = torch.permute(y_scores, (0, 3, 1, 2))
          y_hat_scores_perm = torch.permute(y_hat_scores, (0, 3, 1, 2))
          y_hat_scores_perm = F.interpolate(y_hat_scores_perm, size=y_scores_perm.shape[2:], mode='nearest')
          y_hat_scores = torch.permute(y_hat_scores_perm, (0, 2, 3, 1))
        bce = _neg_loss(y_hat_scores, y_scores)
        
        y_offset = y['offset']
        y_hat_offset = y_hat['offset']
        if distill and y_offset.shape != y_hat_offset.shape:
          bce = torch.nan_to_num(bce, nan=0.0, posinf=0.0, neginf=0.0)
          return bce, {}
        else:
          reg_offset = (y_hat_offset - y_offset).abs().sum(-1).mean(0)

        # SMPL-X params
        rotmat_size = min([y_hat['rotmat'].shape[1], y['rotmat'].shape[1]])
        reg_rotmat = (y_hat['rotmat'][:,:rotmat_size] - y['rotmat'][:,:rotmat_size]).abs().sum([1,2,3]).mean(0) # root/body/hands/jaw
        shape_dim = min([y_hat['shape'].shape[1], y['shape'].shape[1]])
        reg_shape = (y_hat['shape'][:,:shape_dim] - y['shape'][:,:shape_dim]).abs().sum(-1).mean(0)
        if y_hat['dist_postprocessed'].shape == y['dist_postprocessed'].shape:
          reg_dist = (y_hat['dist_postprocessed'] - y['dist_postprocessed']).abs().mean(0)
        else:
        ...
        # for 3DPW dataset
        if j3d.shape[1] != j3d_hat.shape[1]:
          with open(SMPLX2SMPL_REGRESSOR, 'rb') as f:
            smplx2smpl_regressor = torch.from_numpy(pickle.load(f)['matrix'].astype(np.float32)).to(y_hat['j3d'].device)
            v3d_hat = (smplx2smpl_regressor @ v3d_hat)

            J_regressor_h36m = torch.Tensor(np.load('models/smpl/J_regressor_h36m.npy')).to(y_hat['j3d'].device)

            h36m = J_regressor_h36m @ v3d
            h36m_hat = J_regressor_h36m @ v3d_hat
        ...
        # 2D reprojection
        if y['j2d'].shape[1] != y_hat['j2d'].shape[1]:
          # skip for 3DPW
          reg_j2d = torch.tensor(0.0)
          reg_v2d = torch.tensor(0.0)
        else:
